...
(function() {
   ...
   * @property {[string, string]} [key: string]
   */

  /**
   * @typedef {Object} ProcessRoutes
   * @property {ProcessRoute} [key: string]
   */
  ...
  const STATUSES = {
    ...
    ALL_MATERIAL_USED: '素材全量使用済・残寸法ゼロ',
  };
  ...
  const processRoutes = {
    ...
    [OUTSOURCE_TYPES.ONLY_OUTSOURCE]: {
      [STATUSES.NOT_ACCEPTED]: ['受入完了', STATUSES.ACCEPTED],
      [STATUSES.ACCEPTED]: ['外注作業', STATUSES.OUTSOURCING],
  ...
  const handleError = (error, type, context) => {
    ...
    if (CONFIG.ERROR_LOG_ENABLED) {
      console.error(`[${context}] ${response.type}:`, {
  ...
  /**
   * kintoneの保存ボタンをクリックする
   * ※注意: このセレクタはkintoneのUIアップデートで変更される可能性があります。
   */
  const clickSaveButton = () => {
    document.querySelector('.gaia-ui-actionmenu-save')?.click();
  };
  ...
  const callKintoneApi = async (apiCall) => {
    try {
      ...
      throw handleError(error, ERROR_TYPES.API_ERROR, 'KintoneAPI');
    }
  };
  ...
  const updateRecordAndSave = (updates) => {
    try {
      ...
      for (const [fieldCode, value] of Object.entries(updates)) {
        if (eventObj.record[fieldCode]) {
          validateValue(value, fieldCode);
          eventObj.record[fieldCode].value = value;
        }
      }
    ...
    try {
      ...
      const getRes = await callKintoneApi(() => 
        kintone.api(kintone.api.url('/k/v1/records', true), 'GET', {
          app: DEST_APP_ID,
          query: `${FIELDS.STORAGE_ID} = "${managementNo}"`
        })
      );
  ...
  const prepareStorageAppTransition = async (rec) => {
    try {
      ...
      const isZansuZero = rec[FIELDS.REMAINING_DIMENSION_CALC]?.value === STATUSES.ALL_MATERIAL_USED;
      ...
      if (getRes.records.length > 0) {
        ...
        recordId = getRes.records[0].$id.value;
  ...
  const createButton = (label, bgColor, textColor, onClick, hoverColor = null) => {
    try {
      ...
      validateValue(bgColor, '背景色', { required: true, pattern: /^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/ });
   ...
   * @returns {object | null} { caption: 'ボタン名', onClick: 関数 }
   */
  const determineProcessAction = (record) => {
    try {
      ...
      const isBunno = record[FIELDS.SPLIT_DELIVERY_1]?.value.includes('分納') || 
                     record[FIELDS.SPLIT_DELIVERY_2]?.value.includes('分納');
      const isKan = record[FIELDS.FULL_DELIVERY_FLAG]?.value.includes('一時作業停止し保管材へ');
      ...
      const createStatusUpdateAction = (nextStatus) => async () => {
        ...
        const isSazaiSka = currentRecordData[FIELDS.REMAINING_MATERIAL]?.value === 'S化';
        const isFromStorage = currentRecordData[FIELDS.WORK_INFO_2]?.value.includes('保管材からの作業');
        ...
        if (external === OUTSOURCE_TYPES.NONE) {
          if (isBunno) {
            if (!isKan) {
              ...
              if (soukeijouSuuryou && parseFloat(soukeijouSuuryou) !== 0) {
                return {
                  caption: '全ストップ・計上要求',
                  onClick: () => {
  ...
  const createStorageButton = (record) => {
    try {
      ...
      if (zanasi !== '保管' || !validStatuses.includes(status)) {
        return null;
      }
  ...
  const disableBunnoTableRows = (event) => {
    try {
      ...
      if (table) {
        table.forEach((row) => {
          const isShipped = row.value[FIELDS.SPLIT_DELIVERY_STATUS]?.value === STATUSES.SHIPPED;
