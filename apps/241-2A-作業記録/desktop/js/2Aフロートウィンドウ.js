...
(function () {
  ...
  const showLoading = (title = '処理中...', text = 'データを更新しています') => {
    if (hasSwal()) {
      Swal.fire({
        title,
        text,
  ...
  const handleError = (error, type, context) => {
    ...
    closeSwal();
    const detail = error?.message || '予期せぬエラーが発生しました';
    showErrorDialog('エラー', `${userMessage}<br>詳細: ${detail}`);
  };
   ...
   */
  const updateRecordAndSave = (updates, options = {}) => {
    try {
      const {
        loadingTitle = '処理中...',
        loadingText = 'ステータスを更新しています',
        successMessage = 'ステータスを更新しました'
      } = options;

      showLoading(loadingTitle, loadingText);
      setSuccessAfterReload(successMessage);
  ...
  const updateStorageMaterialStatus = async (currentRecord) => {
    ...
    try {
      showLoading("処理中...", "保管ステータスを更新しています");
      ...
      closeSwal();
      await showSuccessToast(`保管材を「${STATUSES.STORAGE_ALL_USED}」に更新しました`);
    } catch (error) {
      closeSwal();
  ...
  const prepareStorageAppTransition = async (rec) => {
    try {
      showLoading("処理中...", "保管アプリのレコードを準備しています");
      ...
      const customerDetailId = rec?.[FIELDS.CUSTOMER_DETAIL_ID]?.value;
      ...
      if (typeof customerDetailId === 'string' && customerDetailId.length > 0) {
        updateRec[FIELDS.DEST_CUSTOMER_LOOKUP] = {
          value: customerDetailId,
          lookup: true
        };
      } else {
        updateRec[FIELDS.DEST_CUSTOMER_LOOKUP] = { value: '' };
      }
  ...
  const promptAndPrepareStorageApp = async () => {
    await showInfo("保管登録", "保管の入力を行ってください。");
            ...
            if (!isKan) {
              ...
              if (soukeijouSuuryou && Number.parseFloat(soukeijouSuuryou) !== 0) {
                return {
                  caption: "総ストップ・計上要求",
                  onClick: async () => {
                    await showWarn("注意", "総計上数量が0になるようにしてください。作業を中断します。");
                    updateRecordAndSave({
  ...
  const createStorageButton = (record) => {
    try {
      ...
      const isEnabled = validStatuses.includes(status);
      const btn = createButton("保管登録", "#97DB34", "black", async () => {
        const rec = kintone.app.record.get().record;
        if (await prepareStorageAppTransition(rec)) {
          showLoading("処理中...", "保管情報を保存しています");
          setSuccessAfterReload("保管情報を保存しました");
          clickSaveButton();
        }
      });
      ...
      if (btn) {
        ...
        if (!isEnabled) {
          btn.disabled = true;
          btn.title = 'プロセスを「作業完了/チェック待ち →」に進めると利用できます。';
  ...
  const showSuccessMessageAfterReload = (event) => {
    try {
      ...
      logError(error, ERROR_TYPES.UNKNOWN_ERROR, "ShowSuccessToast");
    }
    return event;
  };
  ...
  kintone.events.on(['app.record.edit.show', 'app.record.create.show'], showSuccessMessageAfterReload);
  ...
  kintone.events.on('app.record.detail.show', (event) => {
    showSuccessMessageAfterReload(event);
    try {
