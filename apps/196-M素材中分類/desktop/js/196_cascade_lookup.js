/**
 * カスケードルックアップ — アプリ196 M素材中分類
 *
 * Dよみ先頭文字 / D客先グループ のいずれかを選択すると、
 * アプリ195 (MA客先詳細) を絞り込み、カスケードドロップダウン
 * （客先名→所属部署→担当者名）を動的に生成し、
 * 最終選択時に196のフィールドへコピーする。
 *
 * コピーマッピング（既存L客先選択のlookup設定に準拠）:
 *   客先名ID  ← 客先名ID
 *   客先名    ← 客先名
 *   担当者    ← 担当者
 *   素材中分類登録用ID_1 ← 素材中分類登録用ID
 *   客先詳細ID ← 客先詳細ID
 *   L客先選択  ← 客先詳細ID
 *
 * @version 1.0.0
