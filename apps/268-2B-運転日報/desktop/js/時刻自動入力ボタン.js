(function () {
      ...
      const setupButtonCell = (colIndex, targetFieldCode) => {
        if (colIndex !== -1) {
          ...
          if (targetTd && !targetTd.querySelector(`.${BUTTON_CLASS_NAME}`)) {
            // 既存の要素（入力フィールドなど）を非表示にする
            Array.from(targetTd.children).forEach(child => {
              child.style.display = 'none';
            });

            // targetTd.innerHTML = ''; // 既存の要素を消さないようにコメントアウト（または削除）
            const button = createTimeButton(targetFieldCode); // 行インデックスは動的に取得するため渡さない
   ...
   */
  const createTimeButton = (targetTimeFieldCode) => {
    ...
    button.onclick = (e) => {
      ...
      // クリックされたボタンから行インデックスを動的に特定する（行の追加削除に強くする）
      ...
      if (currentRecord.record[TABLE_FIELD_CODE].value[rowIndex]) {
  ...
  const validateArrivalTime = (event) => {
    ...
    const departureTime = row.value[DEPARTURE_FIELD_CODE].value;
    const arrivalTime = row.value[ARRIVAL_FIELD_CODE].value;

    // アラートではなくフィールドのエラーメッセージとして表示
    if (departureTime && arrivalTime && arrivalTime < departureTime) {
      row.value[ARRIVAL_FIELD_CODE].error = '到着時刻が出発時刻より前にセットされました。到着時刻の確認をしてください';
    } else {
      row.value[ARRIVAL_FIELD_CODE].error = null;
    }
  };
